...
double gettan(CvPoint pt)
{
  double angle= atan2((double)pt.y-120,(double)pt.x-80);
  return angle;
...
//////////////////////////////////////////////////////////////////////////////////////
   ...
   {
      if(l_cir_center.x!=0&&l_cir_center.y!=20) // if left point exit
      {
        if (r_cir_center.x!=140&&r_cir_center.y!=20)  // left and right both exist
        {
     ...
     }else // if left not exit
      {
       if(r_cir_center.x!=140&&r_cir_center.y!=20) // if right exit
         {// output right
       cvLine(frame, fb_center, r_cir_center,cvScalar(255,255,0),1,8,0);
       //sendchar(253);sendchar(r_cir_center.x);sendchar(r_cir_center.y);
       //sendchar(254);
       ...
       process(r_cir_center);
         }else if (b_cir_center.x!=0&&b_cir_center.y!=100)  // if right doesnot exit
           {
            ...
            //sendchar(253);sendchar(b_cir_center.x);sendchar(b_cir_center.y);
            //sendchar(254);
