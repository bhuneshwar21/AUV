...
CvScalar black1=cvScalar(50, 30, 30);
CvScalar black2=cvScalar(120, 130, 110);
...
void processArea(CvPoint pt1, CvPoint pt2)
{
 int a_i=0,area_v=0,control;
 int r_length, r_width;
 r_length=abs(pt1.x-pt2.x);
 r_width=abs(pt1.y-pt2.y);
 int Area=r_length*r_width;
 Area_ar[area_count]=Area;
 printf("Area: %d\t",Area);
 area_count++;area_count=area_count%10;
 for (;a_i<10;a_i++)
 {
  area_v=area_v+Area_ar[a_i];
 }
 area_v=area_v/10;
 double rArea_e=sqrt(area_v)-rArea_d;
printf("width %5.2f\t",sqrt(area_v));
printf("errdis: %5.2f\t",rArea_e); 
control=rArea_e*kA+125;
 if (control>255)
  control=255;
 if (control<0)
  control=0;
